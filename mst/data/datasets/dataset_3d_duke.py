...
class DUKE_Dataset3D(data.Dataset):
    ...
    def __init__(
        ...
        self.path_root_data = self.path_root/'preprocessed_crop_n4bc_plhe_fast_full-a/data'
        self.split = split 
        ...
        # Get split file 
        path_csv = self.path_root/'preprocessed_crop_n4bc_plhe_fast_full-a/splits/split.csv'
    ...
    def __getitem__(self, index):
        ...
        if '_' in str(uid):
            ...
            padded_number = parts[0].zfill(3)  # Zero-pad to 3 digits
            formatted_uid = f"{padded_number}_{'_'.join(parts[1:])}"
        else:
            formatted_uid = str(uid).zfill(3)
        
        img = self.load_img(self.path_root_data/f'Breast_MRI_{formatted_uid}'/'sub.nii.gz')
